...
from microninja.gtk3.heading import Heading
from microninja.gtk3.scrolled_window import ScrolledWindow
from microninja.gtk3.apply_styles import apply_common_to_screen, apply_styling_to_widget, apply_colours_to_widget
...
class KanoDialog():
    ...
    def __init__(self, title_text="", description_text="", button_dict=None, widget=None, background_path="",
    ...
    def __colour_dialog_background(self):
        ...
        if len(self.background_path) > 0:
            backgroundimg = Gtk.Image ()
            backgroundimg.set_from_file (self.background_path)
    ...
    def __generate_buttons(self):
        ...
        leftBox = Gtk.Box()
        leftBox.set_size_request(5, -1)
        kano_button_box.pack_start(leftBox, False, False, 6)
        if wasOk == True:
            kano_button_box.pack_start(okButton, False, False, 6)
    ...
    def run(self):
        ...
        self.dialog.set_icon_from_file("/usr/share/icons/microninja/66x66/apps/microninja-dialog.png")
        self.dialog.run()
        self.dialog.destroy()
...
def parse_items(args):
    ...
    return title, description, buttons, widget, background, has_entry, has_list, scrolled_text, global_style, hide_from_taskbar
