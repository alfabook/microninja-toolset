...
from microninja.gtk3 import icons
...
class TopBar(Gtk.EventBox):
    def __init__(self, title, window_width=-1, has_buttons=True):
        ...
        self.close_button.add(self.cross)
        self.close_button.set_size_request(TOP_BAR_HEIGHT, TOP_BAR_HEIGHT)
        self.close_button.set_can_focus(False)
        #this row "almost" solved the problem of the border button in state activate, but
        #remaied the problem with the mouse hover, that made the button glowing.
        #Also tried tho change style for state: Gtk.StateType.PRELIGHT 
        ...
        self.add_style(self.box, "top_bar_container")
    ...
    def disable_prev(self):
        if self.has_buttons:
            ...
            #self.prev_button.remove(self.prev_button.get_children()[0])
            #self.prev_button.add(self.dark_prev_arrow)
    ...
    def disable_next(self):
        if self.has_buttons:
            ...
            #self.next_button.remove(self.next_button.get_children()[0])
            #self.next_button.add(self.dark_next_arrow)
