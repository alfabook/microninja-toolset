...
import re
import shutil
from microninja.utils import run_cmd, get_user_unsudoed, run_bg, write_file_contents
...
def connect(iface, essid, encrypt='off', seckey=None, wpa_custom_file=None):
    ...
    if os.access('/var/run/microninja-connect.pid', os.R_OK):
        client_module=sys.argv[0]
        if client_module.find('microninja-connect') == -1:
            logger.info ('Cancelling microninja-connect to give control to %s' % sys.argv[0])
            run_cmd('pkill -f microninja-connect')
            time.sleep(1)

    udhcpc_cmdline = 'udhcpc -S -t 70 -A 20 -n -a --script=/etc/udhcpc/microninja.script -i %s' % iface
    time.sleep(1)
    ...
    if wpa_custom_file:
        ...
        run_cmd("wpa_supplicant -D nl80211,wext -t -d -c%s -i%s -f /var/log/microninja_wpa.log -B" % (wpa_custom_file, iface))
        ...
        wpafile = '/etc/microninja_wpa_connect.conf'
        associated = False
        if not wpa_conf(essid, seckey, confile=wpafile, wep=True):
            return False

        # wpa_supplicant might complain even if it goes ahead doing its job
        run_cmd("wpa_supplicant -D nl80211,wext -t -d -c%s -i%s -f /var/log/microninja_wpa.log -B" % (wpafile, iface))
