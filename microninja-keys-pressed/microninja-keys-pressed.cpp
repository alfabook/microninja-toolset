...
// Rebadged with microninja - including unistd.h needed for usleep
...
//  License: http://www.gnu.org/licenses/gpl-2.0.txt GNU General Public License v2
//  Added #include <unistd.h> needed for usleep
    ...
    while ((c = getopt(argc, argv, "?hvr:d:")) != EOF)
        {
            switch (c)
                ...
                case '?':
                case 'h':
                    cout << "microninja-keys-pressed [ -? | -h | -v | -r retry_times (default=" << retry << ") ";
